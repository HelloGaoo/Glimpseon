...
int Glimpseon_native::idle_get_milliseconds() {
    LASTINPUTINFO lii = {};
...
int Glimpseon_native::idle_get_seconds() {
...
bool Glimpseon_native::acquire_mutex(const std::string& name) {
...
void Glimpseon_native::release_mutex() {
    if (g_mutex_handle) {
...
int Glimpseon_native::install_font(const std::string& path) {
...
std::tuple<int, int, py::bytes> Glimpseon_native::extract_icon(const std::string& path, int size) {
    std::wstring wpath(path.begin(), path.end());
    SHFILEINFOW sfi = {};
